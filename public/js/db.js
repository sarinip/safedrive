(function ($) {
    ...
    this.getStudentName = function () {
        let studentname = $("#dpdStudent").find(":selected").text();
        if (studentname !== 'Select') {
            $("#studentname").empty();
            $("#studentname").html(studentname);
            getScheduleCount();
        } else {
            $("#studentname").empty();
            $("#studentname").html('Not Selected');
        }
    }
    ...
    this.getScheduleDate = function () {
        let appinmentdate = $("#datepicker").val();
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                new Chart($("#usage"), {
                    ...
                    data: {
                        datasets: [{
                            data: [res.data[0], res.data[1]],  // specify usage percentage here
                            backgroundColor: [
                                'rgba(54, 162, 235, 0.2)',
                                'rgba(255, 99, 132, 0.2)'
                            ],
